...
export function MovePageButton({ pageId, }: { pageId: string, }) {
    ...
    const renderOptions = (node: PageNode, depth = 1): JSX.Element[] => {
        ...
        const options = [<SelectItem key={node.id} value={node.id}>{indent} {node.title}</SelectItem>]
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <button onClick={() => setOpen(true)}>
                    <Move size={16} className="text-gray-500 hover:text-gray-800 cursor-pointer" />
                </button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                ...
                <Select value={newParentId} onValueChange={setNewParentId}>
                    <SelectTrigger><SelectValue placeholder="Select new parent..." /></SelectTrigger>
                    <SelectContent>
