...
func (uc *CreateImportPlanUseCase) Execute(_ context.Context, in CreateImportPlanInput) (*CreateImportPlanOutput, error) {
	if _, err := uc.svc.CreateImportPlanFromZipUpload(in.File, in.TargetBasePath); err != nil {
		// Deliberately static messages, matching ErrCodeImporterUploadTooLarge
		// below: err's wrapped chain (e.g. "extract zip to temp: extract
		// zip: write file: file too large: 200 bytes (max 100)") is internal
		// implementation detail — function names and call-path context, not
		// anything a user should see — so it's logged, never surfaced.
