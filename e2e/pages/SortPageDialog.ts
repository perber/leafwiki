...
export default class SortPageDialog {
  ...
  async getCurrentOrder(): Promise<string[]> {
    ...
    for (let i = 0; i < count; i++) {
      const title = await items.nth(i).locator('span[data-testid^="sort-page-title-"]').innerText();
      titles.push(title);
    }
  ...
  async sortPageItems(plannedOrder: string[]) {
    ...
    for (let i = 0; i < plannedOrder.length; i++) {
      ...
      while (desiredIndex > i) {
        const moveUpButton = this.page.locator(`button[data-testid="move-up-button-${pageId}"]`);
        await moveUpButton.click();

        [titles[desiredIndex - 1], titles[desiredIndex]] = [
          titles[desiredIndex],
          titles[desiredIndex - 1],
        ];
